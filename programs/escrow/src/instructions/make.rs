...
pub struct Make<'info> {
    ...
    #[account(init, payer= maker, seeds = [b"escrow", maker.key().as_ref(), seed.to_le_bytes().as_ref()], space = 8 + Escrow::INIT_SPACE, bump)]
...
impl<'info> Make<'info> {
    pub fn init_make(&mut self, seeds: u64, receive_amount: u64, bumps: &MakeBumps) -> Result<()> {
